...
"""
Postcondition functions for symbolic execution
Implements symbolic operations for x86 instructions
"""
...
def sym_copy(env, dest, source):
    """
    Symbolic copy operation (BIN_COPY)
...
def sym_and(env, dest, source):
    """
    Symbolic AND operation (BIN_AND)
    ...
    """
    # Parse operands
    dest_value, dest_type = parse_operand(dest, env) if dest_type != 'register' else (
    env.get_register(dest), 'register')
    source_value, source_type = parse_operand(source, env)
    ...
    env.set_flag('SF', z3.Extract(result.size() - 1, result.size() - 1, result) == 1)
    env.set_flag('OF', z3.BoolVal(False))  # AND clears OF
    env.set_flag('CF', z3.BoolVal(False))  # AND clears CF


def sym_xor(env, dest, source):
    """
    Symbolic XOR operation (BIN_XOR)
    ...
    env.set_flag('ZF', result == 0)
    env.set_flag('SF', z3.Extract(result.size() - 1, result.size() - 1, result) == 1)
    env.set_flag('OF', z3.BoolVal(False))  # XOR clears OF
    env.set_flag('CF', z3.BoolVal(False))  # XOR clears CF
...
def sym_mov(env, dest, source):
    ...
    Args:
        ...
        source: Source operand
    """
    # MOV uses the symbolic copy operation
            ...
            for part in address_expr.split('+'):
                ...
                if part in env.REG_SIZES:
                    ...
                    if part.startswith('0x'):
                        ...
                        value = int(part)
                    components.append(z3.BitVecVal(value, 64))
...
def sym_add(env, dest, source):
    ...
    # Mark as complex arithmetic if large immediates or variable shifts
    if source_type == 'immediate' and source_value.size() > 16:
        env.mark_complex_arithmetic(dest if dest_type == 'register' else 'temp')
    ...
    env.set_flag('CF', z3.ULT(result, dest_value))  # Carry if result < original value (unsigned)
    env.set_flag('OF', z3.And(
        z3.Extract(result.size() - 1, result.size() - 1, dest_value) == z3.Extract(result.size() - 1, result.size() - 1,
                                                                                   source_value),
        z3.Extract(result.size() - 1, result.size() - 1, result) != z3.Extract(result.size() - 1, result.size() - 1,
                                                                               dest_value)
...
def sym_sub(env, dest, source):
    ...
    env.set_flag('SF', z3.Extract(result.size() - 1, result.size() - 1, result) == 1)
    # Simplified carry and overflow flags
    env.set_flag('CF', z3.ULT(dest_value, source_value))  # Carry if dest < source (unsigned)
    env.set_flag('OF', z3.And(
        z3.Extract(result.size() - 1, result.size() - 1, dest_value) != z3.Extract(result.size() - 1, result.size() - 1,
                                                                                   source_value),
        z3.Extract(result.size() - 1, result.size() - 1, result) != z3.Extract(result.size() - 1, result.size() - 1,
...
def sym_test(env, op1, op2):
    ...
    env.set_flag('ZF', result == 0)
    env.set_flag('SF', z3.Extract(result.size() - 1, result.size() - 1, result) == 1)
    env.set_flag('OF', z3.BoolVal(False))  # TEST clears OF
    env.set_flag('CF', z3.BoolVal(False))  # TEST clears CF
...
def sym_neg(env, operand):
    ...
    """
    # Parse operand
    value, op_type = parse_operand(operand, env)
    ...
    env.set_flag('OF', value == z3.BitVecVal(1 << (value.size() - 1), value.size()))  # OF=1 only if operand=MIN_INT
